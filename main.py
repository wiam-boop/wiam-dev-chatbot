...
def is_model_question(message: str) -> bool:
    """
    يكتشف أسئلة عن النموذج أو الـ AI المستخدم
    ويمنع البوت من الإفصاح عن GPT أو Groq.
    """
...
def chat():
    ...
    # =====================================================
    # Model Question — intercept before sending to LLM
    # =====================================================

    if is_model_question(user_message):

        lang = user_message.lower()
        if any(w in lang for w in ["which", "what", "are you", "powered", "built", "based", "version"]):
            answer = (
                "I'm Wiam Dev AI — powered by advanced AI technology. "
                "I'm not able to share details about the underlying model. 🤖💜"
            )
        else:
            answer = (
                "أنا Wiam Dev AI — مدعوم بتقنية ذكاء اصطناعي متقدمة. "
                "لا أستطيع مشاركة تفاصيل النموذج المستخدم. 🤖💜"
            )
